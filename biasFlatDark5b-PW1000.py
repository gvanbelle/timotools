...
from astropy.io import fits     # added to fix error in line 225 im_hdr = imageFile[0].header
                                # see https://github.com/astropy/astropy/issues/6499
...
fileCheck = Path(calMastersPath / 'combined_dark_240.000.fits')
if fileCheck.exists():
    print('combined_dark_240.000.fits exists, reading.')
    #    combined_bias.read(calibrated_path / 'master_bias.fits')
    combined_dark = ccdp.CCDData.read(calMastersPath / 'combined_dark_240.000.fits', unit="adu")
else:
    print("combined_dark_240.000.fits does not exist, generating.")
...
print("Important: Gain and RN set to PW1000 FLI ML16803 currently.")
...
for filt in flat_filters:
    ...
    if fileCheck.exists():
        ...
        if not ifc_raw.files:
            print(ifc_raw, ": no files found.")
        else:
            print("xxxx", ifc_raw)
	...
	#        combined_bias = ccdp.CCDData.read(calMastersPath / 'master_bias.fits')
            combined_bias = [ccd for ccd in ifc_calMasters.ccds(imagetyp='Bias Frame', combined=True)][0]
	#        combined_bias = {ccd.header[exposure]: ccd for ccd in ifc_calMasters.ccds(imagetyp='Bias Frame', combined=True)}
		...
		#        closest_dark = find_nearest_dark_exposure(reduced, combined_darks.keys())
            	...
            	im_hdr = imageFile[0].header
            	im_exptime = im_hdr['exptime']
            	
            	dk_hdr = combined_dark[0].header
            	dk_exptime = dk_hdr['exptime']
            	
            	good_flat = combined_flats[imageFile.header['filter']]          
            	#
            	# gain & RN numbers here from Stephen Levine email 2021-01-29
            	#
            	reduced = ccdp.ccd_process(imageFile, master_bias=combined_bias, dark_frame=combined_dark,
                                    		master_flat=good_flat, gain=1.32*u.electron/u.adu,
                                    		dark_exposure=dk_exptime*u.second, data_exposure=im_exptime*u.second,
                                    		exposure_unit=u.second, readnoise=25.7, gain_corrected=False,
                                    		dark_scale=True)
