...
print("%-53s"%"Filename", "%-6s"%"Armss", "%-6s"%"Exptm", "%-11s"%"Flux", "%-7s"%"Fluxerr", "%-5s"%"SNR", "%-3s"%"num", "%-3s"%"Flg", "%-6s"%"Xcoord", "%-6s"%"Ycoord", "%-5s"%"ObRad", "%-6s"%"MaxPix")
...
#	print(bkg.globalrms)
	...
	imageObjects = sep.extract(imageData, bkg.globalrms*20, err=bkg.globalrms, filter_kernel=kernel)
#	print(imageObjects)
...
#	print(objectRadius)
    ...
    #	print(ab)
		...
		print("%-53s"%file_name, "%6.3f"%airmass, "%6.2f"%exptime, "%11.1f"%ab[0,indexab], "%7.1f"%ab[1,indexab], "%5.0f"%(ab[0,indexab]/ab[1,indexab]), "%3d"%len(imageObjects), "%3d"%ab[2,indexab], "%6.1f"%xadjusted, "%6.1f"%yadjusted, "%5.2f"%ab[5,indexab], "%6d"%maxPixelVal)
	else:
		print("%-53s"%file_name, "%6.3f"%airmass, "%6.2f"%exptime, "  NoDet")
